...
async def async_catalog(request):
    @sync_to_async
    def fetch_catalog():
        categories = list(Category.objects.prefetch_related("books").order_by("name"))
        return [
            {
                "name": cat.name,
                "slug": cat.slug,
...
def checkout(request):
    ...
    line_items = []
    for item in cart:
        price_cents = int((Decimal(str(item["price"])) * 100).to_integral_value(ROUND_HALF_UP))
        line_items.append(
            {
                "price_data": {
                    "currency": "usd",
    ...
    try:
        session = stripe.checkout.Session.create(
            ...
            mode="payment",
            success_url=(
                request.build_absolute_uri(reverse("shop:payment_success")) + "?session_id={CHECKOUT_SESSION_ID}"
            ),
